...
- 🎥 **多源下载**：支持 MissAV、Jable、HohoJ、Memo、KanAV、AvToday、NetFlav、GGJav 等多个数据源，权重优先级排序
...
- 🛡️ **Flaresolverr 支持**：直接请求遭遇 403/429/503 时自动 fallback 至 Flaresolverr 绕过 Cloudflare 防护
...
│   │   ├── memoDownloader.py
│   │   ├── KanAVDownloader.py
│   │   ├── avtodayDownloader.py
│   │   ├── netFlavDownloader.py
│   │   └── ggjavDownloader.py
│   └── webui/              # WebUI 模块
│       ├── app.py          # Flask 路由 & 验证
│       ├── models.py       # 队列数据库模型
...
| 字段 | 说明 |
|------|------|
| `Enabled` | 是否启用 Flaresolverr fallback（默认 `false`） |
| `URL` | Flaresolverr 服务地址（默认 `http://localhost:8191`） |
| `Timeout` | 请求超时时间（秒，默认 60）|

> Flaresolverr 用于当直接 HTTP 请求返回 **403 / 429 / 503**（Cloudflare 防护）时自动重试。
> 可通过 Docker 快速部署：
> ```bash
...
```
提交番号 → 遍历配置的下载器（按权重降序）
  ├── 直接请求成功 → 下载成功 → 元数据刮削（可选）→ 完成
  ├── 直接请求返回 403/429/503 且 Flaresolverr.Enabled
  │    └── Flaresolverr fallback → 重新尝试解析 → 下载成功 / 继续下一个
  └── 全部失败 → Prowlarr兜底（可选）
       ├── 搜索种子 → AI选择最佳 → 添加下载 → 成功
       └── 搜索失败 → 加入重试队列
```
